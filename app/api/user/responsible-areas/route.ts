...
export async function GET(request: Request) {
    ...
    `, [userId])
        ...
        // 2. Cascade: Remote Regions -> contained EPCIs 
        // AND Remote Regions -> contained Communes (via EPCI relationship usually)
        // We assume Region -> EPCI -> Commune hierarchy.
        ...
        // Expand EPCIs from Regions
        if (allRegionIds.length > 0) {
            // Find EPCIs in these regions
            // EPCI_has_Region (EPCI_idEPCI, Region_idRegion)
        ...
        // Expand Communes from EPCIs (including those just added from Regions)
        // Commune_has_EPCI (Commune_idVille, EPCI_idEPCI)
        // Note: We use unique set logic implicitly or filter later.

        // De-duplicate EPCIs for query
        ...
        if (uniqueEpciIds.length > 0) {
         ...
         `, uniqueEpciIds)
            ...
            epciCommuneRows.forEach((row: any) => {
                // We just add them, we'll de-duplicate later
                allCommuneIds.push(row.Commune_idVille)
            })
        }

        // Now we have lists of IDs, we need to fetch their Details (Names, Postcodes)
